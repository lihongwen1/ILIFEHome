...
public class SlideRecyclerView extends RecyclerView {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (mPosition != INVALID_POSITION) {
                    ...
                    if (mFlingView.getChildCount() == 2) {
                        mMenuViewWidth = mFlingView.getChildAt(1).getWidth()+getResources().getDimensionPixelOffset(R.dimen.dp_20);
                    } else {
                ...
                case MotionEvent.ACTION_UP:
                    if (mMenuViewWidth != INVALID_CHILD_WIDTH) {
                        ...
                        if (mVelocityTracker.getXVelocity() < -SNAP_VELOCITY) {    // 向左侧滑达到侧滑最低速度，则打开
                            mScroller.startScroll(scrollX, 0, mMenuViewWidth - scrollX, 0, 500);
    ...
    public boolean closeMenu() {
            if (mFlingView != null){
                int sx = mFlingView.getScrollX();
                if (sx != 0) {
                    mScroller.startScroll(sx, 0, -sx, 0, 500);
                    postInvalidate();
                    return true;
                }
            }
